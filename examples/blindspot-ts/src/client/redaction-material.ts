...
// like the ink is being burned away by your proximity.
//
// Multiple content layers can declassify sequentially via `layerThresholds`:
// each layer has its own proximity threshold, so the name reveals before
// the address, the address before the metadata, etc.
//
// Inspired by:
// - Exat's proximity-based interaction (cursor distance drives state)
// - Obys' tactile paper feel (texture, lift, physical response)
// - EaseReverseClipMenu's reversible clip (scroll back = re-redact)
...
// ── Content material ──
// Renders the panel content texture. Has a `proximity` uniform that controls
// a subtle brightness/lift effect — the document "warms up" as you approach.
const contentVertexShader = /* glsl */ `
  ...
  void main() {
    ...
    // Subtle lift: panel rises 0.15 units as proximity goes 0→1
    vec3 pos = position;
    // Lift is applied via the model matrix in JS, not here —
    // but we pass proximity for the fragment shader
...
const contentFragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    // Warm up: as proximity increases, slightly brighten and warm the content
    // At distance (proximity=0), content is dim — like reading in low light
    // At close range (proximity=1), content is full brightness
    float brightness = 0.55 + 0.45 * proximity;
    vec3 warmTint = vec3(1.0, 0.97, 0.92); // paper warm
    vec3 color = tex.rgb * brightness;
    // Mix toward warm tint as proximity increases
    color = mix(color * vec3(0.9, 0.88, 0.85), color, 0.3 + 0.7 * proximity);
...
export function createContentMaterial(opts: ContentMaterialOptions): THREE.ShaderMaterial {
...
export function createRedactionMaterial(
  ...
  return new THREE.ShaderMaterial({
    uniforms: {
      ...
      time: { value: 0 },
      inkColor: { value: opts.inkColor ?? new THREE.Color(0.08, 0.07, 0.09) },
      glowColor: { value: opts.glowColor ?? new THREE.Color(0.83, 0.40, 0.23) }, // terracotta
